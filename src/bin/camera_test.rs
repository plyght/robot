use robot_hand::{Result, ObjectDetector};
    ...
    loop {
        ...
        for obj in &objects {
            ...
            imgproc::rectangle(&mut frame, rect, color, 2, imgproc::LINE_8, 0)
                .map_err(|e| robot_hand::HandError::Hardware(format!("Rectangle draw failed: {}", e)))?;
        ...
        let stats = format!("FPS: {:.1} | Objects: {} | Total: {}", fps, objects.len(), total_objects);
        imgproc::put_text(
            &mut frame,
            &stats,
        ...
        if elapsed.as_millis() < 33 {
            std::thread::sleep(std::time::Duration::from_millis(33 - elapsed.as_millis() as u64));
        }
    }
